...
#[no_mangle]
extern "C" fn default_getter () -> u32 {
    return 0;
}
        ...
        if let (&mut Some(ref mut tim3),&mut Some(ref mut task_info_array)) = 
            (MUTEX_TIM3.borrow(cs).borrow_mut().deref_mut(), TASK_INFO_ARRAY.borrow(cs).borrow_mut().deref_mut())
         {
            ...
            else 
            {
                ...
                else if prev_task == 0 { //if it was the idle task
                    next_task = *PAUSED_TASK;
                }
                else {
                    next_task = prev_task + 1;
                }

                //deciding to find awake task or run next idle task
...
fn initialize_task(func_ptr:u32, stack_size: u32) -> u8 {
    ...
    //making sure stack_min will be 4 byte aligned
    if stack_size % 4 != 0 {
        return 1;
    }   
