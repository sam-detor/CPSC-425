...
static TASK_RANGE: Mutex<RefCell<Option<[usize;2]>>> = Mutex::new(RefCell::new(Some([1,0])));
static TASKS_LOADED: Mutex<Cell<usize>> = Mutex::new(Cell::new(0));
static DEBOUNCED: Mutex<Cell<bool>> = Mutex::new(Cell::new(true));

static LINKED_TASKS: Mutex<RefCell<Option<[u32;MAX_TASKS - 1]>>> = Mutex::new(RefCell::new(Some([0,0,0,0])));
static NUM_LINKED_TASKS: Mutex<Cell<usize>> = Mutex::new(Cell::new(0));
static FREE_BLOCKS: Mutex<RefCell<Option<[u32; MAX_TASKS - 1]>>> = Mutex::new(RefCell::new(Some([0,0,0,0])));
...
fn main() -> ! {
    ...
    rcc.ahb1enr.write(|w| w.gpioaen().set_bit().gpiocen().set_bit().gpioden().set_bit());

    //enable syscfg clock
    rcc.apb2enr.write(|w| w.syscfgen().set_bit());

    //Enable tim3 clock
    rcc.apb1enr.write(|w| w.tim2en().set_bit().tim3en().set_bit().tim4en().set_bit());
    ...
    tim2.arr.write(|w| w.arr().bits(500));
    ...
    free(|cs| {
        ...
        if let (
            ...
            if num_tasks == 0 {
                ...
                //If "next_task" is not awake/not loaded, try to find one that is
...
fn initialize_task(func_ptr: u32, task_id: usize){
    ...
    free(|cs| {
        if let (&mut Some(ref mut task_info_array),&mut Some(ref mut task_range), &mut Some(ref mut free_blocks)) =
            (TASK_INFO_ARRAY.borrow(cs).borrow_mut().deref_mut(),
            TASK_RANGE.borrow(cs).borrow_mut().deref_mut(),
            FREE_BLOCKS.borrow(cs).borrow_mut().deref_mut())
        {
            /* //calculates this task's id (this method will only get called if another task can be loaded)
            if task_range[1] == 4 {
                task_id = task_range[0] - 1;
                task_range[0] = task_id;
            }
            else
            {
                task_id = task_range[1] + 1;
                task_range[1] = task_id;
            } */
            ...
            task_info_array[task_id].stack_start = stack_start;
...
fn TIM2() { //triggered every 0.1s, refreshes the pin
    free(|cs| {

        // Obtain all Mutex protected resources
        if let &mut Some(ref mut tim2) = 
            MUTEX_TIM2.borrow(cs).borrow_mut().deref_mut()
         {
            tim2.sr.write(|w| w.uif().clear_bit()); //clear pending interrupt bit
            DEBOUNCED.borrow(cs).set(true);
...
fn EXTI9_5() { //triggered when PC7 is touched
    ...
    cortex_m::interrupt::free(|cs| {
        ...
        if let (&mut Some(ref mut gpiod), &mut Some(ref mut tim2)) = 
...
/* 
                ...
                //assuming oldest task is first in range
                if TASKS_LOADED.borrow(cs).get() > 0 {
                    unload_task(*NEXT_UNLOAD);
...
#[interrupt]
fn EXTI0() { //triggered when button is pressed
    
    static mut NEXT_LOAD: usize = 1;
...
fn init_free_blocks() {
    ...
    free(|cs| {
        if let &mut Some(ref mut free_blocks) =
            FREE_BLOCKS.borrow(cs).borrow_mut().deref_mut()
        {
            for i in 0..(MAX_TASKS - 1) {
...
fn get_linked_tasks() {
    ...
    let mut index = 0;
    free(|cs| {
        if let &mut Some(ref mut linked_tasks) =
            LINKED_TASKS.borrow(cs).borrow_mut().deref_mut()
        {
        //loading tasks
            //unsafe {
                //task 1
...
fn unload_task(task_id: usize) {
    free(|cs| {
        if let (&mut Some(ref mut task_info_array), &mut Some(ref mut free_blocks)) =
            (TASK_INFO_ARRAY.borrow(cs).borrow_mut().deref_mut(), FREE_BLOCKS.borrow(cs).borrow_mut().deref_mut())
        {
            task_info_array[task_id].loaded = false;
            TASKS_LOADED.borrow(cs).set(TASKS_LOADED.borrow(cs).get() - 1);
            for i in 0..(MAX_TASKS - 1) {
                if free_blocks[i] == 0 {
